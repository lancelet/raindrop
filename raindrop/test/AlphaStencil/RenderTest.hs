...
prop_EStartSeg = H.property $ do
  ...
  let (_, logv) = logRenderSegs (Epsilon 1e-5) sz segList
...
prop_EPxAdd = H.property $ do
  ...
  let
    ...
    addToPixel img ix value = Image.setPixel img ix (value + value')
      where value' = Image.getPixel img ix

    resultImage = foldl (\img (ix, value) -> addToPixel img ix value)
                        (Image.new sz 0)
                        pxAdds
...
sampleTri = catMaybes
  ...
  where eps = Epsilon 1e-5

genFloatSegInSize
  :: (H.MonadGen m, H.GenBase m ~ Identity) => Size -> m (Seg Float)
