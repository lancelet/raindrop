...
import           AlphaStencil.Seg      (Epsilon (Epsilon), P (P, ptx),
                                        PxDivision (PxDivision), Seg,
                                        Sign (Minus, Plus), clip, pixelXRange,
                                        pixelYRange, projXArea, pxDivision,
                                        pxDivisionArea, seg, segP, segPoly,
                                        segQ, segSign, unClipSeg)
import           Image                 (I (I), J (J))
import           Image.Loop            (inc, loop)

tests :: H.Group
tests = H.Group "AlphaStencil.Seg"
...
unit_pixelXRange = H.withTests 1 $ H.property $ do
  let
    fromJust = fromMaybe (error "Just expected")
...
unit_clip_error = H.withTests 1 $ H.property $ do
  ...
  case result of
    Left (ErrorCallWithLocation message _) -> message ===
      "clip: segment does not intersect pixel column"
    Right q -> do
...
unit_pxDivisionArea = H.withTests 1 $ H.property $ do
  let
    pd0 = PxDivision (27%65) (12%65) (4%5) (1%5) :: PxDivision Rational
