...
seg (Epsilon eps) p'@(P px' _) q'@(P qx' _) | dx <= eps = Nothing
                                            | otherwise = Just s
 where
  p, q :: P a
  px, py, qx, qy :: a
...
clip :: forall  a . (Ord a, Num a) => I -> Seg a -> ClipSeg a
...
pxDivision
  :: forall  a . (Ord a, Fractional a) => ClipSeg a -> J -> PxDivision a
...
projXArea (ClipSeg (Seg sign (P px _) (P qx _) _)) = c * (qx - px)
 where
  c = case sign of
    Plus  -> 1
    Minus -> -1
